...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        # find closest ghost
        ghost_position = None
        for position_distros in livingGhostPositionDistributions:
            max_position = max(position_distros, key=position_distros.get)
            ghost_position = max_position

        # find action that brings pacman closest to this ghost  
        min_ghost_dist = float("inf")
        best_action = Directions.STOP
        for action in legal:
            successor_position = Actions.getSuccessor(pacmanPosition, action)
            ghost_distance = self.distancer.getDistance(successor_position, ghost_position)
